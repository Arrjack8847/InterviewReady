...
function normalizeStoredScore(
  ...
  const normalizedScale =
    scale === "hundred" || Number(scale) === 100
      ? "hundred"
      : "ten";

  return clampScore(
    normalizedScale === "hundred" || score > 10
      ? score
      : score * 10,
  );
}
...
export function formatReportDuration(
  ...
  const totalMinutes = Math.max(
    1,
    Math.round(Number(durationMs) / 60_000),
  );
...
function getFeedbackRecord(
  ...
  return (answer.feedback || null) as
    | (Partial<Feedback> &
...
function answerStatus(
  ...
  const feedback =
    getFeedbackRecord(answer);

  if (
    !answer.answerText.trim() ||
    feedback?.answerValidity === "blank"
  ) {
    return "empty";
  }
...
function classifyTechnicalQuestionSubtype(
  question: string,
): TechnicalQuestionSubtype {
  const text = String(
    question || "",
  ).toLowerCase();
...
function recommendedStructure(
  ...
  if (category === "Role-Specific") {
    const subtype =
      classifyTechnicalQuestionSubtype(
        question,
      );
...
function categoryLabel(
  ...
  const questionType = String(
    getFeedbackRecord(answer)?.questionType ||
      "general",
  );
...
function buildAnswerReview(
  ...
  const statusAssessment: Record<
    ReportAnswerStatus,
    string
  > = {
    completed:
      summary ||
      `${getAnswerScoreLabel(score) || "This answer"} response overall.`,

    skipped:
      "This question was skipped and was not included in the score.",

    empty:
      "There was not enough response to evaluate this answer reliably.",

    evaluation_pending:
      "Feedback is still being prepared for this saved answer.",

    evaluation_failed:
      "The answer was preserved, but feedback is temporarily unavailable.",

    legacy:
      summary ||
      "This answer uses the feedback available in a legacy report.",
  };
...
function mergeAnswersAndQuestions(
  ...
  const reviews = orderedQuestions.map(
    (question, index) => {
      const answer =
...
function averageLegacyAnswerScore(
...
function buildCategories(
  ...
  const canonical =
    report?.canonicalMetrics;

  const breakdown =
    selectScoreBreakdown(report);

  const weights =
    INTERVIEW_SCORING_CONFIG
      .topLevelWeights[mode];
  ...
  const speechRows =
    canonicalRows(
      canonical?.speechDelivery,
      SPEECH_METRIC_LABELS,
      {
        speakingPace:
          speechWpm === null
            ? undefined
...
function buildPriorities(
  ...
  add(
    {
      ...
      rank:
        REPORT_PRIORITY_ORDER
          .repeatedContentSpecificity,

      evidenceCount:
        specificityQuestions.length,
      ...
      nextStep:
        "End each example with a result, measurable impact, or clear lesson.",
  ...
  add(
    {
      ...
      evidenceCount:
        structureQuestions.length,

      title:
        "Make the answer easier to follow",

      whyItMatters:
        "A clear sequence helps the interviewer understand your decisions and impact.",
      ...
      nextStep:
        "Plan a one-sentence opening, two or three actions, and a concise result.",
  ...
  add(
    {
      ...
      rank:
        REPORT_PRIORITY_ORDER
          .relevanceOrCompleteness,

      evidenceCount:
        relevanceQuestions.length,

      title:
        "Answer the question more directly",

      whyItMatters:
        "A direct opening establishes relevance before supporting details are added.",
      ...
      nextStep:
        "Begin with your main point, then support it with one relevant example.",
  ...
  add(
    {
      ...
      title:
        "Strengthen answer flow",

      whyItMatters:
        "A planned opening and deliberate pauses make spoken answers easier to follow.",

      evidence:
        "Answer flow was below the target coaching range for measurable speech.",

      nextStep:
        "Pause briefly before speaking and state the main point in your first sentence.",
  ...
  add(
    {
      ...
      whyItMatters:
        "Deliberate pacing gives important ideas more space and improves clarity.",
  ...
  add(
    {
      ...
      title:
        "Improve the recording environment",

      whyItMatters:
        "Clear audio helps the system and the listener follow the answer reliably.",

      evidence:
        "Audio clarity was below the target coaching range.",

      nextStep:
        "Use a quieter space and keep a consistent distance from the microphone.",
  ...
  if (!candidates.length) {
    candidates.push({
      ...
      title:
        "Build on your strongest answers",

      whyItMatters:
        "Repeating a strong structure makes good performance more consistent.",

      evidence:
        "No major repeated weakness was detected in the available coaching data.",

      nextStep:
        "Choose your strongest answer and reuse its structure with a different example.",
...
function buildPracticePlan(
  ...
  if (skipped.length) {
    steps.push({
      ...
      title:
        "Complete the skipped questions",

      detail: `Prepare concise responses for question${
        skipped.length === 1
          ? ""
          : "s"
      } ${skipped
  ...
  if (
    ...
    steps.push({
      id: "speech-capture",
      title:
        "Capture a complete spoken answer",
      detail:
        "Check microphone access, then record at least one 60–90 second response.",
    });
  } else if (
    mode === "text"
  ) {
    steps.push({
      ...
      detail:
        "Use voice mode next time to practise delivery after refining the content.",
    });
  }
  ...
  if (
    ...
    steps.push({
      ...
      detail:
        "Allow camera access and keep a clear frame for at least ten measurable seconds.",
    });
  }
...
function buildDeliverySections(
  ...
  const visual:
    ReportDeliverySection | null =
    mode !== "video" ||
    (!visualCategory?.available &&
      !visualMetrics)
      ? null
      : {
          score:
            visualCategory?.score ??
            null,
...
function buildIntegrityNotes(
  report: FinalReport | null,
) {
  const integrity =
    report?.integrityMetrics ||
    report?.canonicalMetrics
      ?.integrity;
  ...
  if (
    ...
    notes.push({
      ...
      text:
        `The interview was paused ${integrity.pauseCount} time${
          integrity.pauseCount ===
          1
            ? ""
            : "s"
        }.`,
  ...
  if (
    Number(
      integrity.noFaceEventCount ||
        0,
    ) > 0 ||
    Number(
      integrity.noFaceDurationMs ||
        0,
    ) > 0
  ) {
    notes.push({
      ...
      text:
        "Some camera time was unavailable because no face was measurable. This was kept separate from the coaching score.",
    });
  }

  if (
    Number(
      integrity.multipleFaceEventCount ||
        0,
    ) > 0 ||
    Number(
      integrity.multipleFaceDurationMs ||
        0,
    ) > 0
  ) {
    notes.push({
      ...
      text:
        "The session paused when more than one face was detected and resumed after camera recovery.",
    });
  }

  if (
    Number(
      integrity.analysisErrorCount ||
        0,
    ) > 0
  ) {
    notes.push({
      ...
      text:
        "Some visual analysis samples were unavailable because of browser or landmark-processing errors.",
    });
  }
...
export function buildInterviewReportViewModel(
  ...
  const recommendedAction =
    practicePlan[0]?.detail ||
    "Repeat the session with a new example.";
