...
export function InterviewSidebar({
  ...
  const submittedIds = new Set(submittedQuestionIds);
    ...
    <aside className="space-y-6">
      <section className="app-panel p-4">
        ...
        <div className="mt-3 space-y-1.5">
          {questions.map((question, questionIndex) => {
            ...
            const active = questionIndex === activeQuestionIndex && !submitted;

            const locked = questionIndex > activeQuestionIndex;
            ...
            return (
              ...
              >
                ...
                >
                  {submitted ? <CheckCircle2 className="h-4 w-4" /> : questionIndex + 1}
                </span>

                <span className="line-clamp-2 leading-relaxed">{question.text}</span>
              </div>
            );
          })}
        </div>
      ...
      <section className="app-panel p-5">
        ...
        <div className="mt-4 space-y-3 text-sm">
          ...
          <ContextRow label="Role" value={setup.targetRole || setup.role || "No role selected"} />

          <ContextRow label="Interview Type" value={setup.type} />

          <ContextRow label="Experience Level" value={setup.difficulty} />
      ...
      <section className="app-panel p-5">
        ...
        <div className="mt-4 space-y-3 text-sm">
          <ContextRow
            label="Speech"
            value={modeLabel === "Text" ? "Not captured" : `${speechClarity}% clarity`}
          />

          <ContextRow
            label="Pace"
            value={modeLabel === "Text" ? "Not captured" : `${wordsPerMinute} wpm`}
          />

          <ContextRow label="Camera" value={cameraStatus} />
        </div>
      </section>
    </aside>
  );
